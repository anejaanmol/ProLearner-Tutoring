...
document.getElementById("user-info-form").addEventListener("submit", function (event) {
    ...
    // Perform actions with the form data (e.g., submit to a server, display a confirmation message)
    ...
    // You may also reset the form if needed
    // event.target.reset();
});
...
function setupInitialMessage() {
    ...
    document.getElementById("question-container").addEventListener("click", function () {
        if (!initialClick) {
            fetchQuestionsData();
            initialClick = true;
...
function fetchQuestionsData() {
    fetch('questions.json')
        ...
        .then(data => {
            questionsData = data;
            // console.log(Object.keys(questionsData).length-20)
...
function handleBackButton() {
    ...
    if (currentQuestionIndex > 0) {
        ...
        var previousQuestion = questionsData[Object.keys(questionsData)[currentQuestionIndex]];
